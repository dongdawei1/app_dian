<template>
	<view class="index-list">
		...
		<view class="index-list4">
			<view>个人介绍 : {{ item.describeOne }}</view>
		</view>
		...
		<view class="index-list4">
			<view>发布时间 : {{ item.updateTime }}</view>
		</view>
		<view class="juzhong">
			<button
				type="warn"
				...
				class="mini-class"
				size="mini"
				@click="getContact(item.id, item.isPublishContact)"
				v-if="item.isPublishContact === 1"
			>
				查询手机号码
			</button>
			<button
